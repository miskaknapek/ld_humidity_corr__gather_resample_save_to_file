...
class gather_resample_metMo_data_make_file:
	...
	basic_file_name_start = "24_hours_data"
	...
	# out file paths ( saved so we can check them later )
	#  ( use the latest one for saving data )
	out_file_paths = []
	...
	def generate_out_filepaths_for_final_files__incl_which_columns_used( self ):
		print("\n>>>> generate_out_filepaths_for_final_files__incl_which_columns_used")
		print("\t --- got general out file path : | "+self.basic_file_path_to_final_file+" |")
		print("\t --- got columns : | "+str( self.gathered_column_names_from_file_export_metadata )+" |")
		print("\t --- and file suffix… | "+str( self.file_name_suffix)+" |")
		self.gathered_column_names_from_file_export_metadata 

		# get time 
		timestamp = self.db_search_starttime.isoformat().split("T")[0]

		# get columns
		columns_listing = "_".join( self.gathered_column_names_from_file_export_metadata )

		# final outpath
		final_outpath = self.basic_file_path_to_final_file+self.basic_file_name_start+"__"+timestamp+"__"+columns_listing+self.file_name_suffix

		self.out_file_paths.append( final_outpath )
	...
	def save_data__locally( self ):
		...
		for file_export_metadata_item in self.desired_data_export_metadata:
			...
			self.generate_out_filepaths_for_final_files__incl_which_columns_used()

			print("\t\t --- file_path : |"+self.out_file_paths[-1]+"|")
			print("\t --- preparing to save - at time "+str( time.time() - starttime )+" - got json of length |"+str( len(curr_export_item__as_json  ))+"|")

			fp = open( self.out_file_paths[-1], "w")
